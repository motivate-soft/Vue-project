...
export default {
  ...
  MOVE_MAILS_TO(state, payload) {
    payload.emailIds.forEach((mailId) => {
      ...
      state.mails[mailIndex].mailFolder === "draft"
    ...
    for (const folder in state.meta.unreadMails.folder) {
      ...
      if (folder == state.mail_filter) {
        ...
        // else reduce unread mails count
          ...
          payload.emailIds.forEach((mailId) => {
            let mailIdIndex =
              state.meta.unreadMails.folder[folder].indexOf(mailId);
            if (mailIdIndex !== -1)
  ...
  UPDATE_UNREAD_META_ON_MOVE(state, payload) {
    ...
    payload.emailIds.forEach((mailId) => {
      ...
      if (mail.unread) {
        const cf_unread_mails =
          state.meta.unreadMails.folder[state.mail_filter];
        if (cf_unread_mails) {
